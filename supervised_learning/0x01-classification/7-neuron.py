...
class Neuron:
    ...
    Methods:
        ...
        cost(self, Y, A):
            ...
            by quantifing the error between predicted values and
            expected values.
        evaluate(self, X, Y):
        ...
        train(self, X, Y, iterations=5000, alpha=0.05):
            Trains the neuron by doing a forward propagation and a gradient
            descent 'iterations' number of times

    Static Methods:
        loss(Y, A):
            calculates the loss in multiple training example using logistic
            regression by quantifing the error between predicted values
            and expected values.
        sigmoid(x):
